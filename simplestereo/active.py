...
class StereoFTP:
    ...
    def getCloud(self, imgObj, radius_factor=0.5, roi=None, unwrappingMethod=None, plot=False):
        ...
        if plot:
            ...
            ### TEMP
            temp = np.angle(ghat)
            plt.plot(np.arange(roi_w), temp[roi_h//2-1,:], label="ghat phase", linestyle='-', color='k')
            temp = np.unwrap(temp, axis=1)
            # And remove jumps along other direction
            temp = np.unwrap(temp, axis=0)
            plt.plot(np.arange(roi_w), temp[roi_h//2-1,:], label="ghat phase unwrapped", linestyle='-', color='orange')
            ###
            '''
